...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(webots_path) = get_webots_path() {
        if let Err(e) = setup_webots_linking(&webots_path) {
            println!("cargo:warning=Failed to setup Webots linking: {}", e);
            generate_stub_bindings(&bindings_src_path, &stub_bindings_path)?;
            println!(
                "cargo:warning=Using checked-in Webots bindings with autogenerated stubs; native runtime linking failed"
            );
        } else {
        ...
        generate_stub_bindings(&bindings_src_path, &stub_bindings_path)?;
        println!(
            "cargo:warning=Using checked-in Webots bindings with autogenerated stubs; Webots installation not found"
        );
    }
