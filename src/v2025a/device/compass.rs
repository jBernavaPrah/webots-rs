...
impl Compass {
    ...
    pub fn enable(&self, step_ms: i32) -> Result<(), SimulatorError> {
        ffi_try!(wb_compass_enable(self.tag, step_ms))?;
        Ok(())
    }

    pub fn disable(&self) -> Result<(), SimulatorError> {
        ffi_try!(wb_compass_disable(self.tag))?;
        Ok(())
    }

    pub fn get_sampling_period(&self) -> Result<i32, SimulatorError> {
        let period = ffi_try!(wb_compass_get_sampling_period(self.tag))?;
        Ok(period)
    }

    pub fn get_lookup_table(&self) -> Result<Vec<f64>, SimulatorError> {
        let size = ffi_try!(wb_compass_get_lookup_table_size(self.tag))? as usize;
        let ptr = ffi_try!(wb_compass_get_lookup_table(self.tag))?;
        let slice = unsafe { std::slice::from_raw_parts(ptr, size) };
        Ok(slice.to_vec())
    }

    pub fn values(&self) -> Result<[f64; 3], SimulatorError> {
        let ptr = ffi_try!(wb_compass_get_values(self.tag))?;
